...
enum partType{EMPTY, LINKER, MODULATOR};
enum partIndex{XCOORD, YCOORD, ZCOORD, TYPE, ROTATION, VISITED};
enum posIndex{POSTYPE, POSROT, POSVISITED};
...
void molInit() {
    for (int i = 0; i < npartMod; i++){
        ...
        parts[i][ROTATION] = 1; // Central molecule only has 1 rotation state
        parts[i][VISITED] = 0;
...
int bondCompatible(enum partType type1, int rot1, enum partType type2, int rot2, int dir) {
    ...
    if (type1 == MODULATOR){
        ...
        switch (dir){
            // This will only depend on rotation of the linker sa central molecules only have 1 rotation
            case 0:
                if (rot1 == 0) return 1;
                else return 0;
            case 1:
        ...
        switch (type){
            case MODULATOR:
                ...
                if (pos[newPos[0]][newPos[1]][newPos[2]][POSTYPE] == LINKER){
                    ...
                    if (bondCompatible(type, rot, pos[newPos[0]][newPos[1]][newPos[2]][POSTYPE], pos[newPos[0]][newPos[1]][newPos[2]][POSROT], i)){
                        energy += CovE;
                    }
                    else{
...
double proposeMove(int rp) {
    ...
    double oldE = getEnergy(parts[rp][XCOORD], parts[rp][YCOORD], parts[rp][ZCOORD], parts[rp][TYPE], parts[rp][ROTATION]);
    double newE = getHypEnergy(parts[rp][XCOORD], parts[rp][YCOORD], parts[rp][ZCOORD], parts[rp][TYPE], parts[rp][ROTATION], randDirection);
    double dE = newE - oldE;
...
double proposeRotation(int rp) {
    if (parts[rp][TYPE] == MODULATOR) {
        // Central molecules only have 1 rotation state so nothing can change
        return 0.0;
    }

    // The molecule must be a linker
    int newRot = (int) gsl_rng_uniform_int(rng, 3);
...
int getNumBonds(int x, int y, int z, enum partType type, int rot) {
    ...
    for (int dir = 0; dir < 6; dir++){
        ...
        if (bondCompatible(type, rot, pos[coords[0]][coords[1]][coords[2]][POSTYPE], pos[coords[0]][coords[1]][coords[2]][POSROT], dir)){
            numBonds++;
        }
    }
    return numBonds;
...
int getTotalCovalent() {
    int numBonds = 0;
    for (int i = 0; i < npart; i++){
        numBonds += getNumBonds(parts[i][XCOORD], parts[i][YCOORD], parts[i][ZCOORD], parts[i][TYPE], parts[i][ROTATION]);
    }
...
void findNearestUnvisited(int part, int nn[][6]) {
    ...
    for (int dir = 0; dir < 6; dir++){
        ...
        getNeighborIndex(coords, dir);
        if (bondCompatible(parts[part][TYPE], parts[part][ROTATION], pos[coords[0]][coords[1]][coords[2]][POSTYPE], pos[coords[0]][coords[1]][coords[2]][POSROT], dir)){
...
void findClusters() {
    ...
    for (int i = 0; i < npart; i++){
        ...
        if (clusterSize != 0){
            ...
            printf("Cluster size %d: %d\t Center: %f %f %f\n", i, clusterSize, centerCoords[0], centerCoords[1], centerCoords[2]);
        }
    }
